...
func RenderTitleBar(w int, currentPage string) string {
	...
	// Seitennamen-Zeile (letzte) linksbündig und rot wie SEW, aktuelle Seite markiert.
	pageNames := "Dashboard  Pull Requests"
	if currentPage == "Dashboard" {
		pageNames = "[Dashboard]  Pull Requests"
	} else if currentPage == "Pull Requests" {
		pageNames = "Dashboard  [Pull Requests]"
	}
	pageColored := titleTextStyle.Foreground(red).Render(pageNames)
	pageLine := prefixColored + beforePad + pageColored + afterPad
	if fill := w - (prefixLen + beforePadLen + len([]rune(pageNames)) + afterPadLen); fill > 0 {
		pageLine += titleTextStyle.Foreground(SlashColor).Render(strings.Repeat("/", fill))
	}
